...
{
	DWORD d = 0;
	HINSTANCE FileHandle = CreateFile(PATH, GENERIC_READ,
...
{
	DWORD w;
	HINSTANCE FileHandle1 = CreateFile(L"SortUser.csv",
...
__declspec(dllexport) U* SortUserArray(U* users, LPDWORD count, LPWSTR str)
{
	...
	// Поиск количества фамилий 
	//по условию для формирования новой структуры
	{
...
__declspec(dllexport) VOID AvgAge(U* users, LPDWORD count)
{
	...
	sprintf(array, "%f", avg);
	wchar_t wc[10];
	mbstowcs(wc, array, 10); // бразование много-байтовой строки в строку широких символовq
	HKEY hKey = NULL;
	//дескриттор ключа реестра (он является структурой)
	if (RegOpenKey(HKEY_CURRENT_USER, NULL, &hKey) != ERROR_SUCCESS) 
	//открываем раздел HKEY_CURRENT_USER
	return 1;
	LPWSTR ptr = wc;
...
__declspec(dllexport) VOID WriteToBuff(U* uses, LPDWORD count)
{
	...
	HANDLE hMenu;
	len = wcslen(buffer) + 1;
	hMenu = GlobalAlloc(GMEM_MOVEABLE, len * sizeof(LPWSTR));// выделяет глобальный блок памяти
		(GlobalLock(hMenu), buffer, len * sizeof(LPWSTR)); // опирующая содержимое одной области памяти в другую.
	GlobalUnlock(hMenu); // разблокировывает данный объект памяти глобальных данных.
	OpenClipboard(0); // открывает буфер обмена для проверки и не дает другим приложениям модифицировать его содержание
	EmptyClipboard(); // очищает буфер обмена и освобождает дескрипторы  данных в нем.
	SetClipboardData(CF_UNICODETEXT, hMenu); // помещает данные в буфер обмена в заданном формате. Окно должно быть текущим владелецем буфера обмена, а приложение  должно вызывать функцию OpenClipboard.
	CloseClipboard(); // акрывает буфер обмена.
}
#pragma endregion